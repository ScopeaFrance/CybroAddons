...
class CrmTeam(models.Model):
    ...
    currency_id = fields.Many2one(
        ...
        string="Currency",
        readonly=True,
        default=lambda self: self.get_currency(),
    )
    ...
    def action_your_target(self):
        tree_res = self.env["ir.model.data"].get_object_reference(
            "sales_team_parent", "sales_target_tree_view"
        )
        x = self.env["ir.model.data"].get_object_reference(
            "sales_team_parent", "action_crm_sales_target_act"
        )

        tree_id = tree_res and tree_res[1] or False
        form_res = self.env["ir.model.data"].get_object_reference(
            "sales_team_parent", "sales_target_form_view"
        )
        ...
        if (
            u_id.has_group("sales_team.group_sale_salesman_all_leads")
            and u_id.has_group("sales_team.group_sale_salesman")
            and u_id.has_group("sales_team.group_sale_manager")
        ):
